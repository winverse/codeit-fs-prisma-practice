...
Prisma Client와 PostgreSQL용 fixture로 사용자 5명과 각 사용자의 게시글을 관계 순서에 맞게 생성합니다. `NODE_ENV=development`와 삭제 가능한 로컬 데이터베이스인지 확인하는 검사, Post→User 초기화 코드는 `src/seed-safety.js`에 제공되어 있습니다. `src/seed.js`는 이 두 함수를 가져와 시딩 전에 실행합니다. 학생은 안전 코드를 변경하지 않고 문제 파일의 `TODO 1`부터 `TODO 4`까지 순서대로 구현합니다.
