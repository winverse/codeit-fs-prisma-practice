...
이 실습의 문제 schema는 프로젝트 실행용 공용 schema와 분리되어 있습니다. `check:01`은 학생이 작성한 문제 schema를 Prisma CLI로 검증하고 해당 schema만으로 Client를 생성한 뒤, 실제 `prisma_practice_blog`에서 User–Post 생성·관계 조회·작성자 삭제 시 cascade까지 확인합니다.
...
```bash
npx prisma validate --schema src/practices/01-prisma-model-relations/answers/prisma/schema.prisma
```
