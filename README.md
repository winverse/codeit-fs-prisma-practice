...
의존성을 설치한 직후에는 각 문제의 핵심 구현이 `TODO`로 남아 있어 확인 명령이 실패하는 것이 정상입니다. 실패 메시지에서 요구 조건을 확인하고 해당 실습 폴더의 문제 파일만 수정하세요.
...
`.nvmrc`와 `package.json#engines`에 지정된 Node.js·npm 실행 환경을 사용합니다.
